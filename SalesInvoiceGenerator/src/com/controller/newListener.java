...
case "DeleteLine":
 ...
 case "createLineCancel":
   ...
   private void LoadFile() {
        JOptionPane.showMessageDialog(frame, "select header file", "Attention", JOptionPane.WARNING_MESSAGE);
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            Invoice inv = new Invoice(invNum, custName, invDate);
            frame.getInvoicesArray().add(inv);
            
            }
            
            JOptionPane.showMessageDialog(frame, "select lines file", "Attention", JOptionPane.WARNING_MESSAGE);
   ...
   Invoice header = findInvoiceByNum(invNum);
...
String headers = "";
        String lines = "";
        for (Invoice header : frame.getInvoicesArray()) {
            headers += header.GetCSVData();
            headers += "\n";
            for (InvoiceLine line : header.getLines()) {
                lines += line.GetCSVData();
                lines += "\n";
            }
        }
        JOptionPane.showMessageDialog(frame, "Save header data", "Attention", JOptionPane.WARNING_MESSAGE);
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(frame, "Save lines data", "Attention", JOptionPane.WARNING_MESSAGE);
...
if (selectedRowIndex >= 0){
    Invoice row = frame.getInvHeaderTableModel().getInvoicesArray().get(selectedRowIndex);
...
try {
    ...
    Invoice invoiceFrame1 = new Invoice(invNum, custName, invDate);
...
double itemPrice = Double.parseDouble(itemPriceStr);
 int headerIndex = frame.getInvoicesTable().getSelectedRow();
        Invoice invoice = frame.getInvHeaderTableModel().getInvoicesArray().get(headerIndex); 
